    ...
    <view className='App'>
      ...
      >
        {images.map((url, index) => {
          ...
          return <view className='image-container' key={index} style={{ animation: isEdge ? undefined : 'none', animationDelay: `${isEdge ? delay : 0}s` }}>
            <HeicImage src={url}/>
          </view>
        })}
      </view>
      <view bindtap={handleAnimateClick} className="animate-button" style={{justifyContent:'center'}}>
        <text >
          Restart
        </text>
      </view>
